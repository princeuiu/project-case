...
$(document).ready(function () {
    var fixed_max_fields = 25; //maximum input boxes allowed
    var fixed_wrapper = $(".fixed_input_fields_wrap"); //Fields wrapper
    var fixed_add_button = $(".fixed_add_field_button"); //Add button ID

    var x = 1; //initlal text box count

    $(fixed_add_button).click(function (elm) {
        elm.preventDefault();
        if(x < fixed_max_fields){
             x++;
            $.post(BASE + 'ajax/costs/getcostlists', null,
                function (e) {
                    if (e != false) {
                        console.log(e);
                        $(fixed_wrapper).append('<div class="control-group"><label class="control-label">Fixed cost | Qty</label><div class="controls"><select name="data[Invoice][cost_id][]" id="InvoiceCostId-'+x+'"></select><div class="input-prepend input-append"><input name="data[Invoice][cost_qty][]" step="any" type="number"></div><button class="btn btn-primary fixed_remove_field" title="Remove this row"><i class="halflings-icon white white minus-sign"></i></button></div></div>'); //add input box
                        var obj = $.parseJSON(e);
                        $(obj).each(function (i) { //populate child options 
                            $('#InvoiceCostId-'+x).append("<option value=\"" + obj[i].id + "\">" + obj[i].name + "</option>");
                        });
                    }
                    else {
                        //$('#myModal').modal('hide');
    ...
    $(fixed_wrapper).on("click", ".fixed_remove_field", function (e) { //user click on remove text
        e.preventDefault();
...
$(document).ready(function () {
    ...
    var wrapper = $(".variable_input_fields_wrap"); //Fields wrapper
    var add_button = $(".variable_add_field_button"); //Add button ID
