    ...
    if(!empty($itemsData)){
        ...
        ?>
            <div class="col-lg-12 sm-align-center mg-all-5px pd-all-5px">
                <?php echo $this->Html->image("items/resize/".$item["Item"]["image"], array('class' => 'img-responsive border-radious-10','url' => array('controller' => 'items', 'action' => 'detail', $item["Item"]["slug"]))); ?>
                <div class="employee-name mg-btm-20">
                    <p><?php 
                        echo $this->Html->link(
                            $item['Item']['name'],
                            array('controller' => 'items', 'action' => 'detail', $item["Item"]["slug"]),
